...
- (UIView *)datePickerView {
    if (!_datePickerView) {
        ...
        UIButton *cancelButton = [[UIButton alloc] initWithFrame:CGRectMake(20, 10, 40, 20)];
        ...
        UIButton *okButton = [[UIButton alloc] initWithFrame:CGRectMake(self.frame.size.width - 52, 10, 40, 20)];
...
//    self.leftCalendarItem.backgroundColor = [UIColor greenColor];
...
//    self.centerCalendarItem.backgroundColor = [UIColor yellowColor];
...
- (void)reloadCalendarItems {
    ...
    //因为分页功能, 所有不会存在零数, endScroll时必是整数
...
- (void)calendarItem:(FDCalendarItem *)item didSelectedDate:(NSDate *)date {
    ...
    if (self.calendarViewDidSelectedDayBlock) {
        self.calendarViewDidSelectedDayBlock(date);
    }
}
