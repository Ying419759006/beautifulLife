...
static float const CIRCLE_THICKNESS = 0.1f;  //厚度
...
//        CGContextRotateCTM(context, -incrementAngle);
        CGContextRotateCTM(context, incrementAngle); // 翻转了颜色
    }
    CGPathRelease(path);
}
...
- (UIColor*)color
{
    colorHue = 1 - colorHue; //为了解决 色环颜色反转后  UIcolor与色环颜色不同步问题
    return [UIColor colorWithHue:(colorHue) saturation:1 brightness:1 alpha:colorAlpha];
}

//新增刷新后背景色
- (UIColor*)refreshedBackGroundColor
{
    CGFloat refreshedBackGroundColorHue = - colorHue;
    return [UIColor colorWithHue:(refreshedBackGroundColorHue) saturation:1 brightness:1 alpha:colorAlpha];
}
...
- (CGRect)hueMarkerRect
{
    ...
    //手柄环增大3.5倍
    int multiple = 3.5;
    return CGRectMake(position.x - thickness*multiple / 2.0f + self.bounds.size.width / 2.0f, position.y - thickness*multiple / 2.0f+ self.bounds.size.height / 2.0f, thickness*multiple, thickness*multiple);
}
...
- (BOOL)gestureRecognizerShouldBegin:(UIGestureRecognizer *)gestureRecognizer
{
    if ( gestureRecognizer == hueGestureRecognizer )
    {
        ...
        //为了用户体验,额外增加触发宽度
        int added = 50;
        return ( (radius - thickness - added) * (radius - thickness - added) < distanceSquared ) && ( distanceSquared <= (radius +added) * (radius+added) );
        
//        return ( (radius - thickness) * (radius - thickness) < distanceSquared ) && ( distanceSquared <= radius * radius ); 原来的
