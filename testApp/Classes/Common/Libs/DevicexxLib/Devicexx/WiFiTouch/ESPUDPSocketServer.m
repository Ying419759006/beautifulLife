...
- (id) initWithPort: (int) port AndSocketTimeout: (int) socketTimeout
{
    ...
    if (self)
    {
        self.delegate = [WiFiTouch sharedTouch];
...
- (NSData *) receiveSpecLenBytes: (int)len
{
    ...
    if (recNumber==len)
    {
        ...
        //新增
        if ([self.delegate respondsToSelector:@selector(espUDPSocketServerGetIpdeviceGetIp:)]) {
            [self.delegate espUDPSocketServerGetIpdeviceGetIp:data];
        }
